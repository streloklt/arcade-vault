...
export function HomeClient({
  ...
  games: Game[];
  recentScores: (ScoreRow & { game: Pick<Game, "id" | "title" | "color"> })[];
  topPlayers: ScoreRow[];
}) {
  useReveal();
          ...
          <div className="activity-card">
            ...
            <div className="ticker">
              {recentScores.map((row, i) => (
                <div
                  key={`${row.game.id}-${row.createdAt}-${i}`}
                  className="tick-row"
                  style={{ animationDelay: i * 60 + "ms" }}
                >
                  <span className={"tk-p neon-" + row.game.color}>
                    {row.name}
                  </span>
                  <span className="tk-mid">▸ {row.game.title}</span>
                  <span className="tk-s">
                    +{row.score.toLocaleString("es-ES")}
                  </span>
                  <span className="tk-t">{formatDate(row.createdAt)}</span>
                </div>
              ))}
            </div>
          </div>

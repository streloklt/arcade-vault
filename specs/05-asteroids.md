...
- **Revertido durante implementación:** el HUD dibujado dentro del canvas (`drawHUD`) ya no dibuja SCORE, NIVEL ni los íconos de vidas — solo conserva el indicador de disparo triple (`3x Ns`). El overlay de `GAME OVER` se mantiene sin cambios. Motivo: el usuario reportó que score/nivel/vidas quedaban duplicados visualmente contra el HUD de React (`.player-hud`), que ya los muestra de forma más legible; se optó por eliminar la redundancia en el canvas en vez de mantener ambos.
